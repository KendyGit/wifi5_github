...
#define USE_SINGLE_AND_CONTINUOUS 1u
...
u8 Key_Prime_SinglePressOK = 0,Key_Prime_ContinuousPressOK = 0;
#else
u8 Key_Prime_ShortPressOK = 0,Key_Prime_LongPressOK = 0;
#endif
u8 Key_Prime_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Prime_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Prime_Short_Long(void)
  {
    KeyScan();
  }
#endif
...
static u8 Step_KeyReleasing(void)
{
  ...
  {
    Key_Prime_ContinuousPressOK = 0;		  //实验
//	Key_Prime_LongPressOK = 0;	   //实验
	pressTimeCount = 0;
    #if USE_SINGLE_AND_CONTINUOUS
