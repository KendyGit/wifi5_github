...
u8 Key_Time_SinglePressOK = 0,Key_Time_ContinuousPressOK = 0;
#else
u8 Key_Time_ShortPressOK = 0,Key_Time_LongPressOK = 0;
#endif
u8 Key_Time_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Time_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Time_Short_Long(void)
  {
    KeyScan();
  }
#endif
