...
u8 Key_Down_SinglePressOK = 0,Key_Down_ContinuousPressOK = 0;
#else
u8 Key_Down_ShortPressOK = 0,Key_Down_LongPressOK = 0;
#endif
u8 Key_Down_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Down_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Down_Short_Long(void)
  {
    KeyScan();
  }
#endif
