...
u8 Key_Set_SinglePressOK = 0,Key_Set_ContinuousPressOK = 0;
#else
u8 Key_Set_ShortPressOK = 0,Key_Set_LongPressOK = 0;
#endif
u8 Key_Set_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Set_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Set_Short_Long(void)
  {
    KeyScan();
  }
#endif
...
static void KeyScan(void)
{
  ...
  key_Status = KEY_Set;// 获取按键的状态KEY_FC;//
