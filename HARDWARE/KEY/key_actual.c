...
u8 Key_Actual_SinglePressOK = 0,Key_Actual_ContinuousPressOK = 0;
#else
u8 Key_Actual_ShortPressOK = 0,Key_Actual_LongPressOK = 0;
#endif
u8 Key_Actual_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Actual_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Actual_Short_Long(void)
  {
    KeyScan();
  }
#endif
    ...
    if(SET == key_Status)
    {
      ...
      else
      {
        if(EXECUTE_TIME == pressTimeCount)
        {
          pressTimeCount = 0;
          // 连击标志置为有效
          Key_Actual_ContinuousPressOK = 1;
