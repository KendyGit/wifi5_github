...
u8 Key_Onoff_SinglePressOK = 0,Key_Onoff_ContinuousPressOK = 0;
#else
u8 Key_Onoff_ShortPressOK = 0,Key_Onoff_LongPressOK = 0;
#endif
u8 Key_Onoff_DoublePressOK = 0;
...
enum _KEY_STATUS{
  ...
  static u8 continuous_PressTimeout = 0;//  是否到达连按时间标志
    // 按键核验后处理
  void BSP_Key_Onoff_Single_Continuous(void)
  {
    KeyScan();
  }
#else
  void BSP_Key_Onoff_Short_Long(void)
  {
    KeyScan();
  }
#endif
...
/*****状态3 第三个10ms检测到按下及状态4 第三个10ms检测放开******/
  ...
  static u8 Step_WaitForRelease(void)
  {
    ...
    {
      pressTimeCount++;
//	  if(pressTimeCount >= LONG_PRESS_FLAG) pressTimeCount=LONG_PRESS_FLAG;//防止计数溢出 /***/
...
static u8 Step_KeyReleasing(void)
{
  ...
  {
	Key_Onoff_LongPressOK = 0;		 //实验
	pressTimeCount = 0;
    #if USE_SINGLE_AND_CONTINUOUS
      continuous_PressTimeout = 0;
    #endif
  }
//	Key_Onoff_LongPressOK = 0;
  return waitForPress;
}
/***定义步骤****/
...
static void KeyScan(void)
{
  ...
  key_Status = KEY_OnOff;//KEY_OnOff;// 获取按键的状态
